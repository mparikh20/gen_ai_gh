...
def select_pubmed_data(data_df,
    ...
    if num_rows is None:
        ...
        data_df1 = data_df[data_df[['pmid','abstract']].notna().all(axis=1)][cols]

    else:
        data_df1 = data_df[data_df[['pmid','abstract']].notna().all(axis=1)][cols].sample(n=num_rows,random_state=random_state)

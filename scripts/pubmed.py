...
def query_and_ids(search_output):
    ...
    results_df['acquisition_date'] = today_date

    print('\tMetadata obtained and saved in a df.')
...
def get_abstracts(search_output,
    ...
    Args:
        ...
        max_records = max. number of records to obtain.
                      This argument necessary only when ids_from_server is True.
    Returns:
        A list of all PMIDs relevant to the search query.
    ...
    if ids_from_server:
        ...
        fetchurl_suffix = f'efetch.fcgi?db={database}&query_key={query_key}&WebEnv={web_env}&rettype={content_type}&retstart={starting_index}&retmax={max_records}&retmode=xml'

        # Get list of all ids:
        ids_suffix = f'efetch.fcgi?db={database}&query_key={query_key}&WebEnv={web_env}&rettype=uilist&retmode=text'
...
def run_pubmed_pipeline(query,
    ...
    print('Collecting metadata about the search results into a df.')
    metadata_df = query_and_ids(search_output)
